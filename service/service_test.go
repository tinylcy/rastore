...
func (t *testStore) Join(nodeID string, addr string) error {
	return nil
}
...
func TestNewServer(t *testing.T) {
	...
	if err := s.Start(); err != nil {
		t.Fatalf("faile to start HTTP service: %s", err)
	}
	...
	b = doGet(t, s.URL(), "k1")
	if string(b) != `{"k1":"v1"}` {
		t.Fatalf(`wrong value received for key k1: %s (expectd "v1")`, string(b))
	}
...
func doGet(t *testing.T, url string, key string) string {
...
func doPost(t *testing.T, url string, key string, value string) {
	b, err := json.Marshal(map[string]string{key: value})
	if err != nil {
		t.Fatalf("failed to encode key and value for POST: %s", err)
	}
	resp, err := http.Post(fmt.Sprintf("%s/key", url), "application/json", bytes.NewReader(b))
	if err != nil {
...
func doDelete(t *testing.T, u string, key string) {
